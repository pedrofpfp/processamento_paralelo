...
struct HashMap {
    ...
    // Busca sem alocação — retorna ID ou -1
    int find(const char* s, size_t len) const {
        uint32_t h = fnv1a(s, len);
        int idx = (int)(h & (uint32_t)mask);
        while (table[idx].id != EMPTY) {
            if (table[idx].hash == h) {
                const auto& k = keys[table[idx].id];
                if (k.size() == len && !memcmp(k.data(), s, len)) return table[idx].id;
            }
            idx = (idx + 1) & mask;
        }
        return -1;
...
//
// Para cada coluna categórica: grava {coluna}_mapa.csv com valor → ID → freq.
// Grava dados_encoded.csv com categorias substituídas pelo ID inteiro.
//
// Estratégia:
//   - Mapa: paralelo por coluna (cada thread escreve seu próprio arquivo)
//   - CSV encodado: chunks paralelos produzem buffers → escrita sequencial em disco
//     (escrita paralela com pwrite seria mais rápida, mas perde ordem)
// =============================================================================
static void gerar_arquivos(const Mapa& m, size_t pos0, char delim, const Ctx& ctx) {
    int NC = (int)ctx.cols.size();
    int nt = omp_get_max_threads();

    // --- Passo 1: um arquivo de mapeamento por coluna categórica (paralelo) ---
    #pragma omp parallel for schedule(dynamic, 1) num_threads(nt)
    for (int c = 0; c < NC; c++) {
        if (ctx.cols[c].num) continue;
        std::string fname = ctx.cols[c].nome + "_mapa.csv";
        FILE* f = fopen(fname.c_str(), "w");
        if (!f) continue;
        fprintf(f, "valor,id,frequencia\n");
        const HashMap& d = ctx.cols[c].dict;
        size_t sz = d.size();
        // Escreve em ordem decrescente de frequência (selection sort — sz geralmente pequeno)
        std::vector<size_t> idx(sz); for (size_t i = 0; i < sz; i++) idx[i] = i;
        for (size_t i = 0; i < sz; i++) {
            size_t best = i;
            for (size_t j = i+1; j < sz; j++)
                if (d.cnts[idx[j]] > d.cnts[idx[best]]) best = j;
            if (best != i) { size_t tmp = idx[i]; idx[i] = idx[best]; idx[best] = tmp; }
            fprintf(f, "\"%s\",%zu,%zu\n",
                    d.keys[idx[i]].c_str(), idx[i], d.cnts[idx[i]]);
        }
        fclose(f);
        #pragma omp critical(print)
        std::cout << "  Mapa gerado: " << fname << " (" << sz << " valores)\n";
    }

    // --- Passo 2: CSV encodado — categorias → IDs inteiros ---
    FILE* out = fopen("dados_encoded.csv", "w");
    if (!out) { std::cerr << "ERRO: nao criou dados_encoded.csv\n"; return; }

    // Cabeçalho
    for (int c = 0; c < NC; c++)
        fprintf(out, "%s%c", ctx.cols[c].nome.c_str(), c < NC-1 ? delim : '\n');

    const size_t WCHUNK = 300'000;
    size_t pos = pos0, written = 0;

    while (pos < m.tam) {
        // Coleta offsets do chunk
        std::vector<size_t> offs;
        offs.reserve(WCHUNK);
        size_t p = pos;
        while (p < m.tam && offs.size() < WCHUNK) {
            offs.push_back(p);
            while (p < m.tam && m.buf[p] != '\n') p++;
            if (p < m.tam) p++;
        }
        ...
        // Threads constroem cada linha em paralelo → buffers de string
        std::vector<std::string> bufs(NL);
        ...
        for (size_t li = 0; li < NL; li++) {
            ...
            size_t p1 = (li+1 < NL) ? offs[li+1] : (pos < m.tam ? pos : m.tam);
            int col = 0; size_t f = p0;
            std::string line; line.reserve(NC * 6);
            ...
            while (f < p1 && col < NC) {
                ...
                if (ctx.cols[col].num) {
                    line.append(sv, sl);        // numérica: copia como está
                } else {
                    int id = is_null(sv, sl) ? -1 : ctx.cols[col].dict.find(sv, sl);
                    char nbuf[16]; snprintf(nbuf, sizeof(nbuf), "%d", id);
                    line += nbuf;               // categórica: escreve o ID inteiro
                }
                if (col < NC - 1) line += delim;
                f = g + 1; col++;
            }
            line += '\n';
            bufs[li] = std::move(line);
        }

        // Escrita sequencial em disco (garante ordem das linhas)
        for (size_t li = 0; li < NL; li++)
            fwrite(bufs[li].data(), 1, bufs[li].size(), out);

        written += NL;
        std::cout << "> Encodando " << written << " linhas...\r" << std::flush;
    }
    std::cout << "\n";
    fclose(out);
    std::cout << "  CSV gerado : dados_encoded.csv\n";
}
...
int main(int argc, char* argv[]) {
    ...
    // Gera arquivos ANTES de fechar o mmap (encoded CSV precisa relê-lo)
    std::cout << "\nGerando arquivos de saida...\n";
    auto t3 = now();
    gerar_arquivos(m, pos_dados, DELIM, ctx);
    double t_files = secs(now() - t3);
